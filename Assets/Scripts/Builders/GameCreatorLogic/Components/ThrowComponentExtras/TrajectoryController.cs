        ...
        for (int i = 0; i < resolution - 1; i++)
        {
            if (i < resolution - 1)
            {
                ...
                if (Physics.Linecast(points[i], points[i + 1], out hitInfo))
                {
                    ...
                    if (hitInfo.collider.CompareTag("Item") || hitInfo.collider.CompareTag("Untagged") || hitInfo.collider.CompareTag("Footsteps/grass") || hitInfo.collider.CompareTag("Footsteps/floor"))//"Ground"
                    {
