...
public class FeedsManager : MonoBehaviour
{
    ...
    IEnumerator WaitToStartCallForFeedScene()
    {
       ...
       Debug.Log("FeedsManager isLoginFromDifferentId:" + SNS_APIResponseManager.Instance.isLoginFromDifferentId);
    ...
    IEnumerator WaitToSceneLoad()
    {
        ...
        // END OLD FEED UI
        //SNS_APIResponseManager.Instance.RequestGetAllFollowers(1, 10, "FeedStart");//Get All Follower
        // OLD FEED UI
        ...
        //}
        // END Old UI
        //Debug.Log("FeedsManager Start:" + Application.internetReachability);
        ...
        if (Application.internetReachability == NetworkReachability.NotReachable)
        {
            ...
            {
                SNS_APIResponseManager.Instance.LoadJson();
            }

            if (File.Exists(Application.persistentDataPath + "/FeedFollowingData.json"))
            {
                SNS_APIResponseManager.Instance.LoadJsonFollowingFeed();
            }
        }
        else
        {
    ...
    public void OnFeedButtonTabBtnClick()
    {
       Debug.Log("OnFeedButtonTabBtnClick.......");
        SNS_APIResponseManager.Instance.OnFeedAPiCalling();
    ...
    public void OnClickFollowingTabBtnClick()
    {
        ...
        //SNS_APIResponseManager.Instance.RequestGetFeedsByFollowingUser(1, 10);
        //}
    }
    ...
    public void OnClickHotAndDiscoverTabBtnClick()
    {
        ...
        //{
        //isAnyUserFollow = false;
        //SNS_APIResponseManager.Instance.RequestGetAllUsersWithFeeds(1, 5);
        //}
    }

    public void ResetAllFeedScreen(bool isFeedScreen)
    {
        if (isFeedScreen && SNS_APIResponseManager.Instance.allUserRootList.Count == 0)
        {
        ...
        {
            OtherUserProfileManager.Instance.backKeyManageList.Clear();
        }

        SNSSettingManager.Instance.myAccountScreen.SetActive(false);
        SNSSettingManager.Instance.myAccountPersonalInfoScreen.SetActive(false);
    ...
    {
        SNS_APIResponseManager.Instance.ClearAllFeedDataForLogout();
        ...
        SNS_APIController.Instance.feedHotIdList.Clear();
        MyProfileManager.Instance.myProfileData = new GetUserDetailData();
        MyProfileManager.Instance.allMyFeedImageRootDataList.Clear();
        MyProfileManager.Instance.allMyFeedVideoRootDataList.Clear();
        if (OtherUserProfileManager.Instance != null)
        {
            OtherUserProfileManager.Instance.allMyFeedImageRootDataList.Clear();
            OtherUserProfileManager.Instance.allMyFeedVideoRootDataList.Clear();
            OtherUserProfileManager.Instance.backKeyManageList.Clear();
        }
        ...
        if (MyProfileManager.Instance.allPhotoContainer)
        {
            foreach (Transform item in MyProfileManager.Instance.allPhotoContainer)
            {
                Destroy(item.gameObject);
            }
        }

        if (MyProfileManager.Instance.allMovieContainer)
        {
            foreach (Transform item in MyProfileManager.Instance.allMovieContainer)
            {
                Destroy(item.gameObject);
            }
        }
    ...
    public void OnClickCheckOtherPlayerProfile(bool _callFromFindFriendWithName=false)
    {
        ...
        //OtherUserProfileManager.Instance.gameObject.SetActive(true);
        ...
        //{
        //    if (OtherUserProfileManager.Instance.visitedUserProfileAssetsData.userOccupiedAssets.Count > 0)
        //    {
        //        //print("user occupied assets data here: " + OtherUserProfileManager.Instance.visitedUserProfileAssetsData.userOccupiedAssets.Count + "::::" + OtherUserProfileManager.Instance.singleUserProfileData.userOccupiedAssets[0].json);
        //        ProfileScreenController.instance.SetUserAvatarClothing(OtherUserProfileManager.Instance.visitedUserProfileAssetsData.userOccupiedAssets[0].json);
        //    }
        //    else
        //    {
        ...
        if (OtherUserProfileManager.Instance.backKeyManageList.Count > 0)
        {
            switch (OtherUserProfileManager.Instance.backKeyManageList[OtherUserProfileManager.Instance.backKeyManageList.Count - 1])
            {
                case "FollowerFollowingListScreen":
                    ...
                    //MyProfileManager.Instance.myProfileScreen.SetActive(false);
                    profileFollowerFollowingListScreen.SetActive(false);
    ...
    public void APiPagination()
    {
        ...
        if (feedUiScrollRectFasterEx.verticalNormalizedPosition <= 0.01f && isDataLoad)
        {
            ...
            {
                SNS_APIResponseManager.Instance.RequestGetAllUsersWithFeeds((allFeedCurrentpage + 1), 10);
            }
            //Debug.Log("scrollRect pos :" + feedUiScrollRectFasterEx.verticalNormalizedPosition);
            if (feedUiHorizontalSnap.CurrentPage == 1)
            {
                //Debug.Log("Feed Following scrollRect pos :" + feedUiScrollRectFasterEx.verticalNormalizedPosition + " rows count:"+ SNS_APIResponseManager.Instance.followingUserRoot.Data.Rows.Count);
                //if (SNS_APIResponseManager.Instance.followingUserRoot.Data.Rows.Count > 0 && followingFeedImageLoadedCount >= (followingFeedInitiateTotalCount - 1))
                if (SNS_APIResponseManager.Instance.followingUserRoot.Data.Rows.Count > 0 /*&& followingFeedInitiateTotalCount < 2*/)
                {
                    isDataLoad = false;
                   Debug.Log("isDataLoad False");
                    SNS_APIResponseManager.Instance.RequestGetFeedsByFollowingUser((followingUserCurrentpage + 1), 10);
                }
            }
            else
            {
                //Debug.Log("Feed scrollRect pos :" + feedUiScrollRectFasterEx.verticalNormalizedPosition + " rows count:"+ SNS_APIResponseManager.Instance.root.data.rows.Count + " :current screen:" + feedUiHorizontalSnap.CurrentPage);
                //Riken
                //if (SNS_APIResponseManager.Instance.root.data.rows.Count > 0)
        ...
        switch (feedFullViewScreenCallingFrom)
        {
            case "MyProfile":
               Debug.Log("Full Video Screen back calling from MyProfile");
                MyProfileManager.Instance.mainPostContainer.gameObject.SetActive(true);
                MyProfileManager.Instance.ResetMainScrollDefaultTopPos();
                MyProfileManager.Instance.SetupEmptyMsgForPhotoTab(false);//check for empty message.......
                break;
            case "OtherProfile":
                OtherUserProfileManager.Instance.mainPostContainer.gameObject.SetActive(true);
    ...
    public void OnClickSearchUserButton()
    {
        ...
        SNS_APIResponseManager.Instance.GetBestFriend();
    ...
    public void OnValueChangeFindFriend()
    {
            ...
            {
                //SNS_APIResponseManager.Instance.RequestGetSearchUser(findFriendInputField.text);
                SNS_APIResponseManager.Instance.RequestGetSearchUser(findFriendInputFieldAdvanced.Text);
    ...
    public void OnValueChangedProfileSearch()
    {
        ...
        if (!string.IsNullOrEmpty(profileFinfFriendAdvancedInputField.Text))
        {
            ...
            {
                //SNS_APIResponseManager.Instance.RequestGetSearchUser(findFriendInputField.text);
                SNS_APIResponseManager.Instance.RequestGetSearchUserForProfile(profileFinfFriendAdvancedInputField.Text);
    ...
    public void CreateFeedAPICall(string url, string thumbnail)
    {
        switch (imageOrVideo)
        {
            case "Image":
                ...
                SNS_APIResponseManager.Instance.RequestCreateFeed(SNS_APIResponseManager.EncodedString(s1), SNS_APIResponseManager.EncodedString(s2), url, "", thumbnail, "true", "", "MyProfileCreateFeed");
                break;
            case "Video":
                ...
                SNS_APIResponseManager.Instance.RequestCreateFeed(SNS_APIResponseManager.EncodedString(s11), SNS_APIResponseManager.EncodedString(s22), "", url, thumbnail, "true", "", "MyProfileCreateFeed");
                break;
            default:
                break;
        }
                ...
                if (path != null)
                {
                    ...
                    switch (NativeGallery.GetMediaTypeOfFile(path))
                    {
                        ...
                        case NativeGallery.MediaType.Video:
                            ...
                            break;
                        default:
                            //SNSWarningsHandler.Instance.ShowWarningMessage("Only upload image or video");
    ...
    public void RemoveUnFollowedUserFromFollowingTab(string callingFrom = "")
    {
        ...
        {
            SNS_APIResponseManager.Instance.FeedFollowingSaveAndUpdateJson(unFollowedUserListForFollowingTab, callingFrom);
        }
    }
    #endregion
    ...
    {
        RemoveUnFollowedUserFromFollowingTab();
        MyProfileManager.Instance.MyProfileSceenShow(true);
        profileFollowerFollowingListScreen.SetActive(false);
    }
    ...
    public void ProfileGetAllFollower(int pageNum)
    {
        ...
        //Debug.Log("ProfileGetAllFollower:" + SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows.Count + "    :pageNum:" + pageNum);
        if (SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows.Count > 0)
        {
            for (int i = 0; i <= SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows.Count; i++)
            {
                if (i < SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows.Count)
                {
                    if (!profileFollowerLoadedItemIDList.Contains(SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows[i].follower.id))
                    {
                        GameObject followerObject = Instantiate(followerPrefab, profileFollowerListContainer);
                        followerObject.GetComponent<FindFriendWithNameItem>().SetupData(SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows[i]);
                        //profileFollowerItemControllersList.Add(followerObject.GetComponent<FollowerItemController>());
                        //profileFollowerLoadedItemIDList.Add(SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows[i].follower.id);
                    }
                }
                //else
                //{
                ...
                //    {
                //        GameObject followerObject = Instantiate(followerPrefab, profileFollowerListContainer);
                //        followerObject.GetComponent<FindFriendWithNameItem>().SetupData(SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows[0], true);
                //    }
                //}
            }
            if (SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows.Count > 10)
            {
                GameObject extra = Instantiate(FeedsManager.Instance.ExtraPrefab, profileFollowerListContainer);
            }
        }
        else
        {
    ...
    IEnumerator WaitToProfileFollowerDataLoad(int pageNum)
    {
      ...
      if (pageNum > 1 && SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows.Count > 0)
        {
    ...
    {
        //Debug.Log("ProfileGetAllFollowing:" + SNS_APIResponseManager.Instance.profileAllFollowingRoot.data.rows.Count + "    :pageNum:" + pageNum);
        for (int i = 0; i < SNS_APIResponseManager.Instance.profileAllFollowingRoot.data.rows.Count; i++)
        {
            if (!profileFollowingLoadedItemIDList.Contains(SNS_APIResponseManager.Instance.profileAllFollowingRoot.data.rows[i].following.id))
            {
                GameObject followingObject = Instantiate(followingPrefab, profileFollowingListContainer);
                followingObject.GetComponent<FollowingItemController>().SetupData(SNS_APIResponseManager.Instance.profileAllFollowingRoot.data.rows[i]);
                profileFollowingItemControllersList.Add(followingObject.GetComponent<FollowingItemController>());
                profileFollowingLoadedItemIDList.Add(SNS_APIResponseManager.Instance.profileAllFollowingRoot.data.rows[i].following.id);
            }
        }
    ...
    {
        //Debug.Log("ProfileGetAllFollowing:" + SNS_APIResponseManager.Instance.profileAllFollowingRoot.data.rows.Count + "    :pageNum:" + pageNum);
        for (int i = 0; i < SNS_APIResponseManager.Instance.AdFrndFollowingRoot.data.rows.Count; i++)
        {
            if (!adFrndFollowingLoadedItemIDList.Contains(SNS_APIResponseManager.Instance.AdFrndFollowingRoot.data.rows[i].following.id))
            {
                GameObject followingObject = Instantiate(followingPrefab, adFrndFollowingListContainer);
                followingObject.GetComponent<FollowingItemController>().SetupData(SNS_APIResponseManager.Instance.AdFrndFollowingRoot.data.rows[i]);
                AdFrndFollowingItemControllersList.Add(followingObject.GetComponent<FollowingItemController>());
                adFrndFollowingLoadedItemIDList.Add(SNS_APIResponseManager.Instance.AdFrndFollowingRoot.data.rows[i].following.id);
            }
        }
    ...
    IEnumerator WaitToProfileFollowingDataLoad(int pageNum)
    {
        ...
        if (pageNum > 1 && SNS_APIResponseManager.Instance.profileAllFollowingRoot.data.rows.Count > 0)
        {
    ...
    IEnumerator WaitToAdFrndFollowingDataLoad(int pageNum)
    {
        ...
        if (pageNum > 1 && SNS_APIResponseManager.Instance.AdFrndFollowingRoot.data.rows.Count > 0)
        {
    ...
    public void ProfileFollowerPaginationAPICall()
    {
        ...
        if (profileFFScreenScrollrectFasterEXList[0].verticalNormalizedPosition <= 0.01f && isProfileFollowerDataLoaded)
        {
            if (SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows.Count > 0)
            {
                ...
                isProfileFollowerDataLoaded = false;
                SNS_APIResponseManager.Instance.RequestGetAllFollowersFromProfile(MyProfileManager.Instance.myProfileData.id.ToString(), (profileFollowerPaginationPageNo + 1), 50);
            }
        }
    }
    ...
    public void ProfileFollowingPaginationAPICall()
    {
        ...
        if (profileFFScreenScrollrectFasterEXList[1].verticalNormalizedPosition <= 0.01f && isProfileFollowingDataLoaded)
        {
            if (SNS_APIResponseManager.Instance.profileAllFollowingRoot.data.rows.Count > 0)
            {
                ...
                isProfileFollowingDataLoaded = false;
                SNS_APIResponseManager.Instance.RequestGetAllFollowingFromProfile(MyProfileManager.Instance.myProfileData.id.ToString(), (profileFollowingPaginationPageNo + 1), 50);
            }
        }
    }
    #endregion
    ...
    public void SetupEditDeleteFeedScreen()
    {
        ...
        {
            bool isUrlContainsHttpAndHttps = SNS_APIResponseManager.Instance.CheckUrlDropboxOrNot(feedEditOrDeleteData.feedImage);
    ...
    public void OnClickEditDeleteFeedEditButton()
    {
        ...
        {
            editFeedDescriptionInputField.Text = SNS_APIResponseManager.DecodedString(feedEditOrDeleteData.feedDescriptions);
        }
        else
        {
    ...
    {
        //ShowLoader(true);
        SNS_APIResponseManager.Instance.RequestDeleteFeed(feedEditOrDeleteData.feedId.ToString(), "DeleteFeed");
    ...
    void DeleteFeedAfterRemoveAndRefreshData()
    {
        ...
        int PostIndex = MyProfileManager.Instance.loadedMyPostAndVideoIdInFeedPage.FindIndex(o => o.Equals(feedEditOrDeleteData.feedId));
        int HotPostIndex = SNS_APIController.Instance.feedHotIdList.FindIndex(o => o.Equals(feedEditOrDeleteData.feedId));
        SNS_APIController.Instance.feedHotIdList.Remove(feedEditOrDeleteData.feedId);
        MyProfileManager.Instance.loadedMyPostAndVideoId.Remove(feedEditOrDeleteData.feedId);
        MyProfileManager.Instance.loadedMyPostAndVideoIdInFeedPage.Remove(feedEditOrDeleteData.feedId);
        AllFeedByUserIdRow allFeedByUserIdForAPI = SNS_APIResponseManager.Instance.allFeedWithUserIdRoot.Data.Rows.Find(x => x.Id.Equals(feedEditOrDeleteData.feedId));
        SNS_APIResponseManager.Instance.allFeedWithUserIdRoot.Data.Rows.Remove(allFeedByUserIdForAPI);

        //for image.......
        if (!string.IsNullOrEmpty(feedEditOrDeleteData.feedImage))
        {
            AllFeedByUserIdRow allFeedByUserIdRow = MyProfileManager.Instance.allMyFeedImageRootDataList.Find((x) => x.Id == feedEditOrDeleteData.feedId);
            Debug.Log("Image Before");
            int imageIndex = MyProfileManager.Instance.allMyFeedImageRootDataList.IndexOf(allFeedByUserIdRow);
            Debug.Log("Image " + imageIndex);
            if (MyProfileManager.Instance.allPhotoContainer.childCount > imageIndex)
            {
                DestroyImmediate(MyProfileManager.Instance.allPhotoContainer.GetChild(imageIndex).gameObject);
                Debug.Log("Image Delete");
            }
            MyProfileManager.Instance.allMyFeedImageRootDataList.Remove(allFeedByUserIdRow);
        }
        else
        {
            AllFeedByUserIdRow allFeedByUserIdRow = MyProfileManager.Instance.allMyFeedVideoRootDataList.Find((x) => x.Id == feedEditOrDeleteData.feedId);
            Debug.Log("Video Before");
            int videoIndex = MyProfileManager.Instance.allMyFeedVideoRootDataList.IndexOf(allFeedByUserIdRow);
            Debug.Log("Video " + videoIndex);
            if (MyProfileManager.Instance.allMovieContainer.childCount > videoIndex)
            {
                DestroyImmediate(MyProfileManager.Instance.allMovieContainer.GetChild(videoIndex).gameObject);
                Debug.Log("Video Delete");
            }
            MyProfileManager.Instance.allMyFeedVideoRootDataList.Remove(allFeedByUserIdRow);
        }

        HotFeed myhotfeed = SNS_APIResponseManager.Instance.allhotFeedRoot.data.rows.Find((x) => x.id == feedEditOrDeleteData.feedId);
        Debug.Log("Hot Image Before");
        int index = SNS_APIResponseManager.Instance.allhotFeedRoot.data.rows.IndexOf(myhotfeed);
        Debug.Log("Hot Image " + index);
        SNS_APIResponseManager.Instance.allhotFeedRoot.data.rows.Remove(myhotfeed);
        ...
        // OLD FEED UI
        //if (SNS_APIResponseManager.Instance.allFeedWithUserIdRoot.Data.Rows.Count == 0)
        //{
        ...
        editDeleteFeedScreen.GetComponent<PopupbehaviourHandler>().ClosePopUp();

        //SNSNotificationHandler.Instance.ShowNotificationMsg("Post deleted");//this method is used to show SNS notification.......

        MyProfileManager.Instance.RequestGetUserDetails();

        if (videofeedParent.childCount <= 0)
        {
            //MyProfileManager.Instance.totalPostText.text = "0";
            OnClickVideoItemBackButton();
        }
    }
    ...
    public void OnClickEditFeedDoneButton()
    {
        ...
        if (updatefeedDescription == 1)
        {
            ...
            ShowLoader(true);
            SNS_APIResponseManager.Instance.RequestEditFeed(feedEditOrDeleteData.feedId.ToString(), SNS_APIResponseManager.EncodedString(lastUpdatedFeedDescriptionStr), feedEditOrDeleteData.feedImage, feedEditOrDeleteData.feedVideo);
        }
        editFeedScreen.SetActive(true);
    }
    ...
    public void OnSuccessFeedEdit()
    {
        ...
        if (!string.IsNullOrEmpty(feedEditOrDeleteData.feedImage))
        {
            ...
            int index = MyProfileManager.Instance.allMyFeedImageRootDataList.IndexOf(allFeedByUserIdRow);
            if (MyProfileManager.Instance.allPhotoContainer.childCount > index)
            {
                MyProfileManager.Instance.allPhotoContainer.GetChild(index).GetComponent<UserPostItem>().userData.Descriptions = lastUpdatedFeedDescriptionStr;
            }
        ...
        else
        {
            ...
            int index = MyProfileManager.Instance.allMyFeedVideoRootDataList.IndexOf(allFeedByUserIdRow);
            if (MyProfileManager.Instance.allMovieContainer.childCount > index)
            {
                MyProfileManager.Instance.allMovieContainer.GetChild(index).GetComponent<UserPostItem>().userData.Descriptions = lastUpdatedFeedDescriptionStr;
            }
            editDeleteCurrentPostFeedVideoItem.userData.Descriptions = lastUpdatedFeedDescriptionStr;
            editDeleteCurrentPostFeedVideoItem.descriptionText.text = lastUpdatedFeedDescriptionStr;
        }
       ...
       Debug.Log("CommentSuccessAfterUpdateRequireFeedResponse:" + feedFullViewScreenCallingFrom);
        switch (feedFullViewScreenCallingFrom)
        {
            case "MyProfile":
                ...
                {
                    //for
                    AllFeedByUserIdRow allFeedByUserIdRow = MyProfileManager.Instance.allMyFeedImageRootDataList.Find((x) => x.Id == postFeedVideoItem.userData.Id);
                    allFeedByUserIdRow.commentCount = postFeedVideoItem.userData.commentCount;

                    int index = MyProfileManager.Instance.allMyFeedImageRootDataList.IndexOf(allFeedByUserIdRow);
                    if (MyProfileManager.Instance.allPhotoContainer.childCount > index)
                    {
                        MyProfileManager.Instance.allPhotoContainer.GetChild(index).GetComponent<UserPostItem>().userData.commentCount = allFeedByUserIdRow.commentCount;
                    }
                }
                else
                {
                    AllFeedByUserIdRow allFeedByUserIdRow = MyProfileManager.Instance.allMyFeedVideoRootDataList.Find((x) => x.Id == postFeedVideoItem.userData.Id);
                    allFeedByUserIdRow.commentCount = postFeedVideoItem.userData.commentCount;

                    int index = MyProfileManager.Instance.allMyFeedVideoRootDataList.IndexOf(allFeedByUserIdRow);
                    if (MyProfileManager.Instance.allMovieContainer.childCount > index)
                    {
                        MyProfileManager.Instance.allMovieContainer.GetChild(index).GetComponent<UserPostItem>().userData.commentCount = allFeedByUserIdRow.commentCount;
                    }
                }
            ...
            case "FeedPage":
                ...
                {
                    //for
                    AllFeedByUserIdRow allFeedByUserIdRow = MyProfileManager.Instance.allMyFeedImageRootDataList.Find((x) => x.Id == postFeedPageVideoItem.userData.Id);
                ...
                {
                    AllFeedByUserIdRow allFeedByUserIdRow = MyProfileManager.Instance.allMyFeedVideoRootDataList.Find((x) => x.Id == postFeedPageVideoItem.userData.Id);
                    allFeedByUserIdRow.commentCount = postFeedPageVideoItem.userData.commentCount;
            ...
            case "OtherProfile":
                ...
                {
                    AllFeedByUserIdRow allFeedByUserIdRow = OtherUserProfileManager.Instance.allMyFeedImageRootDataList.Find((x) => x.Id == postFeedVideoItem1.userData.Id);
                    allFeedByUserIdRow.commentCount = postFeedVideoItem1.userData.commentCount;

                    int index = OtherUserProfileManager.Instance.allMyFeedImageRootDataList.IndexOf(allFeedByUserIdRow);
                    if (OtherUserProfileManager.Instance.userPostParent.childCount > index)
                    {
                        OtherUserProfileManager.Instance.userPostParent.GetChild(index).GetComponent<UserPostItem>().userData.commentCount = allFeedByUserIdRow.commentCount;
                    }
                }
                else
                {
                    AllFeedByUserIdRow allFeedByUserIdRow = OtherUserProfileManager.Instance.allMyFeedVideoRootDataList.Find((x) => x.Id == postFeedVideoItem1.userData.Id);
                    allFeedByUserIdRow.commentCount = postFeedVideoItem1.userData.commentCount;

                    int index = OtherUserProfileManager.Instance.allMyFeedVideoRootDataList.IndexOf(allFeedByUserIdRow);
                    if (OtherUserProfileManager.Instance.allMovieContainer.childCount > index)
                    {
                        OtherUserProfileManager.Instance.allMovieContainer.GetChild(index).GetComponent<UserPostItem>().userData.commentCount = allFeedByUserIdRow.commentCount;
                    }
                }
    ...
    void CommentAfterRefereshFollowingFeedItemResponse()
    {
        ...
        FeedsByFollowingUserRow feedsByFollowingUserRow = SNS_APIResponseManager.Instance.allFollowingUserRootList.Find((x) => x.Id == feedFollowingItemController.FollowingUserFeedData.Id);
    ...
    void CommentAfterRefereshDiscoverAndHotFeedItemResponse()
    {
        ...
        SNS_APIResponseManager.Instance.CommentCountTextSetup(feedVideoItem.hotFeed.commentCount);
        ...
        {
           HotFeed hotFeed = SNS_APIResponseManager.Instance.allhotFeedRoot.data.rows.Find((x) => x.id == feedVideoItem.hotFeed.id);
        ...
        {
            AllUserWithFeedRow allUserWithFeedRow = SNS_APIResponseManager.Instance.allUserRootList.Find((x) => x.id == feedVideoItem.FeedRawData.id);
       ...
       Debug.Log("LikeDislikeSuccessAfterUpdateRequireFeedResponse:" + feedFullViewScreenCallingFrom + "  :isLike:" + isLike + "  :LikeCount:" + likeCount);
        switch (feedFullViewScreenCallingFrom)
        {
            case "MyProfile":
                ...
                int index = SNS_APIResponseManager.Instance.allFeedWithUserIdRoot.Data.Rows.FindIndex(x => x.Id == postFeedVideoItem.userData.Id);
                SNS_APIResponseManager.Instance.allFeedWithUserIdRoot.Data.Rows[index].LikeCount = likeCount;
                SNS_APIResponseManager.Instance.allFeedWithUserIdRoot.Data.Rows[index].isLike = isLike;
                //for image.......
                if (!string.IsNullOrEmpty(postFeedVideoItem.userData.Image))
                {
                    //for
            ...
            case "FeedPage":
                ...
                {
                    //for
                    AllFeedByUserIdRow allFeedByUserIdRow = MyProfileManager.Instance.allMyFeedImageRootDataList.Find((x) => x.Id == postFeedVideoItemFeedPage.userData.Id);
                ...
                {
                    AllFeedByUserIdRow allFeedByUserIdRow = MyProfileManager.Instance.allMyFeedVideoRootDataList.Find((x) => x.Id == postFeedVideoItemFeedPage.userData.Id);
                    allFeedByUserIdRow.isLike = postFeedVideoItemFeedPage.userData.isLike;
                }
    ...
    void FeedCommentAndLikeSocketEventAfterRefereshFollowingFeedItemResponse(int feedId, int likeCount, int commentCount, bool isComesFromLike, bool isFullFeedScreen)
    {
        ...
        if (SNS_APIController.Instance.feedFollowingIdList.Contains(feedId))
        {
            //Debug.Log("Following tab feed data updated");
            FeedsByFollowingUserRow feedsByFollowingUserRow = SNS_APIResponseManager.Instance.allFollowingUserRootList.Find((x) => x.Id == feedId);
            if (feedsByFollowingUserRow != null)
            {
                if (isComesFromLike)
                {
            ...
            if (isFullFeedScreen)
            {
                int index = SNS_APIController.Instance.feedFollowingIdList.IndexOf(feedId);
    ...
    void FeedCommentAndLikeSocketEventAfterRefereshDiscoverAndHotFeedItemResponse(int feedId, int createdBY, int likeCount, int commentCount, bool isComesFromLike, bool isFullFeedScreen)
    {
        ...
        if (SNS_APIController.Instance.feedHotIdList.Contains(feedId))
        {
           Debug.Log("here 1");
            //if (SNS_APIController.Instance.feedForYouIdList.Contains(feedId))
            //{
           ...
           Debug.Log("createdBY => " + createdBY);
            ...
            /*
            AllUserWithFeedRow allUserWithFeedRow = SNS_APIResponseManager.Instance.allUserRootList.Find((x) => x.UserProfile.id == createdBY);
            ...
            */
            ...
            if (isFullFeedScreen)
            {
                ...
                //   int index = SNS_APIController.Instance.feedForYouIdList.IndexOf(feedId);
                // Debug.Log("Index:" + index);
                //current full feed selected item data update.......
                ...
                else
                {
                    ...
                    if (commentPanel.activeInHierarchy)//if comment screen is open then refresh comment list api.......
                    {
                        if (SNS_APIResponseManager.Instance.feedIdTemp == feedVideoItem.FeedData.id)
                        {
    ...
    void FeedCommentAndLikeSocketEventAfterRefereshMyProfileResponse(int feedId, int likeCount, int commentCount, bool isComesFromLike, bool isFullFeedScreen)
    {
       Debug.Log("FeedCommentAndLikeSocketEventAfterRefereshMyProfileResponse feed ID:" + feedId + "  :LikeCount:" + likeCount + "    :CommentCount:" + commentCount + "  :IsComesFromLike:" + isComesFromLike + "    :IsFullScreen:" + isFullFeedScreen);
        if (MyProfileManager.Instance.loadedMyPostAndVideoId.Contains(feedId))
        {
            AllFeedByUserIdRow allFeedByUserIdRow = MyProfileManager.Instance.allMyFeedImageRootDataList.Find((x) => x.Id == feedId);
            if (allFeedByUserIdRow == null)
            {
                allFeedByUserIdRow = MyProfileManager.Instance.allMyFeedVideoRootDataList.Find((x) => x.Id == feedId);
            }
            //Debug.Log("here");
            if (allFeedByUserIdRow != null)
            {
                ...
                if (isFullFeedScreen)
                {
                    ...
                    {
                        index = MyProfileManager.Instance.allMyFeedImageRootDataList.IndexOf(allFeedByUserIdRow);
                    }
                    else
                    {
                        index = MyProfileManager.Instance.allMyFeedVideoRootDataList.IndexOf(allFeedByUserIdRow);
                    }
                    ...
                    if (index < videofeedParent.childCount)
                    {
                        ...
                        if (postFeedVideoItem.userData.Id == feedId)
                        {
                            ...
                            else
                            {
                                ...
                                if (commentPanel.activeInHierarchy)//if comment screen is open then refresh comment list api.......
                                {
                                    if (SNS_APIResponseManager.Instance.feedIdTemp == feedId)
                                    {
        ...
        if (MyProfileManager.Instance.loadedMyPostAndVideoIdInFeedPage.Contains(feedId))
        {
            AllFeedByUserIdRow allFeedByUserIdRow = SNS_APIResponseManager.Instance.allFeedWithUserIdRoot.Data.Rows.Find((x) => x.Id == feedId);
            //Debug.Log("here");
            if (allFeedByUserIdRow != null)
            {
                //Debug.Log("my profile feed data updated");
                ...
                int index;
                index = SNS_APIResponseManager.Instance.allFeedWithUserIdRoot.Data.Rows.IndexOf(allFeedByUserIdRow);
                if (isComesFromLike)
                {
                    SNS_APIResponseManager.Instance.allFeedWithUserIdRoot.Data.Rows[index].LikeCount = likeCount;
                }
                ...
                {
                    SNS_APIResponseManager.Instance.allFeedWithUserIdRoot.Data.Rows[index].commentCount = commentCount;
                }
            }
        }
    }
    ...
    void FeedCommentAndLikeSocketEventAfterRefereshOtherUserProfileResponse(int feedId, int likeCount, int commentCount, bool isComesFromLike, bool isFullFeedScreen)
    {
       Debug.Log("FeedCommentAndLikeSocketEventAfterRefereshOtherUserProfileResponse feed ID:" + feedId + "  :LikeCount:" + likeCount + "    :CommentCount:" + commentCount + "  :IsComesFromLike:" + isComesFromLike + "    :IsFullScreen:" + isFullFeedScreen);
        if (OtherUserProfileManager.Instance.loadedMyPostAndVideoId.Contains(feedId))
        {
            AllFeedByUserIdRow allFeedByUserIdRow = OtherUserProfileManager.Instance.allMyFeedImageRootDataList.Find((x) => x.Id == feedId);
            if (allFeedByUserIdRow == null)
            {
                allFeedByUserIdRow = OtherUserProfileManager.Instance.allMyFeedVideoRootDataList.Find((x) => x.Id == feedId);
            }
            ...
            if (allFeedByUserIdRow != null)
            {
                ...
                if (isFullFeedScreen)
                {
                    ...
                    if (!string.IsNullOrEmpty(allFeedByUserIdRow.Image))
                    {
                        index = OtherUserProfileManager.Instance.allMyFeedImageRootDataList.IndexOf(allFeedByUserIdRow);
                    }
                    else
                    {
                        index = OtherUserProfileManager.Instance.allMyFeedVideoRootDataList.IndexOf(allFeedByUserIdRow);
                    }
    ...
    public void OnClickHotFrnd()
    {
     ...
     public void OnClickRecommedationFrnd(){
        if (!AddFrndRecommendedPanel.activeInHierarchy){ 
            ...
            findFriendInputFieldAdvanced.Text = "";
            findFriendScreen.SetActive(false);
            SNS_APIResponseManager.Instance.SetRecommendedFriend();
    ...
    public void OnClickMutalFrnd(){
         if (!AddFrndMutalFrndPanel.activeInHierarchy){ 
            ...
            findFriendInputFieldAdvanced.Text = "";
            findFriendScreen.SetActive(false);
            SNS_APIResponseManager.Instance.SetMutalFrndList();
    ...
    public void OnClickProfileFollowerButton() 
    {
        ...
        SNS_APIResponseManager.Instance.RequestGetAllFollowersFromProfile(SNS_APIResponseManager.Instance.userId.ToString(), 1, 50);
    ...
    public void OnClickProfileFollowingButton()
    {
        ...
        profileFinfFriendScreen.SetActive(false);
        SNS_APIResponseManager.Instance.RequestGetAllFollowingFromProfile(SNS_APIResponseManager.Instance.userId.ToString(), 1, 50);
        ProfileFollowerFollowingScreenSetup(1, "");
        //FeedsManager.Instance.OnClickProfileFollowingButton();
    }
    /// <summary>
