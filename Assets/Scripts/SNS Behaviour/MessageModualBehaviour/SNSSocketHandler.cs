...
public class SNSSocketHandler : MonoBehaviour
{
    ...
    public void MessageReceivedResponse(string s)
    {
        ...
        //  SNS_APIResponseManager.Instance.isCreateMessage = true;
        msgResponce = JsonConvert.DeserializeObject<SocketResponce>(s);
    ...
    public IEnumerator IERequestChatGetMessagesSocket(int message_pageNumber, int message_pageSize, int message_receiverId, int message_receivedGroupId)
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Post((address + ConstantsGod.r_url_ChatGetMessages), form))
        {
            ...
            else
            {
                ...
                SNS_APIResponseManager.Instance.allChatMessagesRoot = JsonConvert.DeserializeObject<ChatGetMessagesRoot>(data, settings);
    ...
    public void FeedCommentResponse(string s)
    {
        ...
        FeedsManager.Instance.FeedCommentAndLikeSocketEventSuccessAfterUpdateRequireFeedResponse(feedCommentSocketRoot.feedId, feedCommentSocketRoot.createdBy, 0, feedCommentSocketRoot.commentCount, false);
    }
    ...
    public void FeedLikeResponse(string s)
    {
       ...
       Debug.Log("SNS_APIResponseManager.Instance.userId" + SNS_APIResponseManager.Instance.userId);
       Debug.Log("SNS_APIResponseManager.Instance.userId" + SNS_APIResponseManager.Instance.userId);

        //if (feedLikeSocketRoot.createdBy == SNS_APIResponseManager.Instance.userId)
        //    return;
        // LikeDislikeSuccessAfterUpdateRequireFeedResponse
        FeedsManager.Instance.FeedCommentAndLikeSocketEventSuccessAfterUpdateRequireFeedResponse(feedLikeSocketRoot.feedId, feedLikeSocketRoot.createdBy, feedLikeSocketRoot.likeCount, 0, true);
    }
    #endregion
}
