...
public class MyProfileManager : MonoBehaviour
{
    ...
    public UserRolesBadgeHandler userRolesView;
    ...
    {
        SNS_APIResponseManager.Instance.RequestGetUserDetails("myProfile");//Get My Profile data       
    ...
    public void SetupData(GetUserDetailData myData, string callingFrom)
    {
        ...
        {
            LoadDataMyProfile();//set data
            SNS_APIResponseManager.Instance.RequestGetFeedsByUserId(SNS_APIResponseManager.Instance.userId, 1, 40, "MyProfile");
        }
    }
    ...
    {
        //Old photo and video based feed implementation
    ...
    public void LoadDataMyProfile()
    {
        ...
        {
            topHaderUserNameText.GetComponent<SetPrefferedWidth>().SetupObjectWidth();
        }
        ...
        if (myProfileData.userProfile != null)
        {
            ...
            //    {
            //        jobText.text = SNS_APIResponseManager.DecodedString(myProfileData.userProfile.job);
            ...
            {
                textUserBio.text = SNS_APIResponseManager.DecodedString(myProfileData.userProfile.bio);
                if (textUserBio.text == " ")
                    _alignment_space.SetActive(false);
                else
        ...
        //    bool isUrlContainsHttpAndHttps = SNS_APIResponseManager.Instance.CheckUrlDropboxOrNot(myProfileData.avatar);
        ...
        if (myProfileScreen.activeSelf)
        {
            ...
            if (profileMainScrollRectFasterEx.verticalNormalizedPosition < 0.01f && isFeedLoaded)
            {
                ...
                if (currentPageAllFeedWithUserIdRoot.Data.Rows.Count > 0)
                {
                    ...
                    SNS_APIResponseManager.Instance.RequestGetFeedsByUserId(SNS_APIResponseManager.Instance.userId, (profileFeedAPiCurrentPageIndex + 1), 10, "MyProfile");
                }
                // OnScrollNFT();
            }
        }
    ...
    public void AllFeedWithUserId(int pageNumb, Transform Feedparent = null, bool IsNew = false)
    {
        ...
        //currentPageAllFeedWithUserIdRoot = SNS_APIResponseManager.Instance.allFeedWithUserIdRoot;
        ...
        ////        FeedsManager.Instance.AllFeedScreenMessageTextActive(true, 2, UITextLocalization.GetLocaliseTextByKey("There's nothing to show here."));
        ////    }
        ////    else
        ////    {
        ...
        currentPageAllTextPostWithUserIdRoot = SNS_APIResponseManager.Instance.allTextPostWithUserIdRoot;
        bool IsMyProfileFeed = false;
        //FeedsManager.Instance.ShowLoader(false);
        ...
        for (int i = 0; i <= currentPageAllTextPostWithUserIdRoot.data.rows.Count; i++)
        {
            if (i < currentPageAllTextPostWithUserIdRoot.data.rows.Count)
            {
                  ...
                  /* && (currentPageAllTextPostWithUserIdRoot.data.rows[i].text_post.ToLower() != "null")*/)
                {
                    ...
                    //    Debug.Log("userTagPostObject is Instantiate in FeedParent");
                    //}
                    //if (SNS_APIResponseManager.Instance.allTextPostWithUserIdRoot.data.rows.Count == 0)
                    //{
                    //    FeedsManager.Instance.AllFeedScreenMessageTextActive(true, 2, UITextLocalization.GetLocaliseTextByKey("There's nothing to show here."));
                    //}
                    //else
    ...
    public IEnumerator AllTagFeed()
    {
        ...
        for (int i = 0; i < SNS_APIResponseManager.Instance.taggedFeedsByUserIdRoot.data.rows.Count; i++)
        {
            ...
            //Debug.Log("tagdata" + SNS_APIResponseManager.Instance.taggedFeedsByUserIdRoot.data.rows[i]);
            UserPostItem userPostItem = userPostObject.GetComponent<UserPostItem>();
            userPostItem.tagUserData = SNS_APIResponseManager.Instance.taggedFeedsByUserIdRoot.data.rows[i];
    ...
    public void CheckAndDisableFirstFeedPopupForMyNFT(bool isMyNFTScreen)
    {
        ...
        if (isMyNFTScreen)
        {
            if (SNS_NFTHandler.Instance.ContentPanel.transform.childCount > 0)
            {
                createYourFirstPostMsgObj.SetActive(false);
                FooterCreateIcon.GetComponent<Animator>().enabled = false;
                FooterCreateIcon.transform.GetChild(0).transform.rotation = Quaternion.Euler(0f, 0f, 0f);
    ...
    public void OnClickOtherPalyerProfileBackButton()
    {
        ...
        FeedsManager.Instance.otherPlayerProfileScreen.SetActive(false);
    }

    //this method is used to follow user button click.......
    public void OnClickFollowUserButton()
    {
        SNS_APIResponseManager.Instance.RequestFollowAUser(FeedRawData.id.ToString(), "MyProfile");
    }

    //this method is used to Create post Button Click.......
    public void OnClickCreatePostButton()
    {
        FeedsManager.Instance.OnClickCreateFeedPickImageOrVideo();
    }
    ...
    public void OnClickPhotoTabButtonMain(int index)
    {
        ...
        SNS_NFTHandler.Instance.nftloading.SetActive(false);
        SNS_NFTHandler.Instance.ResetNftData();
        SNS_NFTHandler.Instance.NftLoadingPenal.SetActive(false);
    }
    ...
    { 
        ...
        if (SNS_NFTHandler.Instance.ContentPanel.transform.childCount <= 0)
        {
            SNS_NFTHandler.Instance.NftLoadingPenal.SetActive(true);
        }
        else
        {
            SNS_NFTHandler.Instance.NftLoadingPenal.SetActive(false);
        }
        //SNS_NFTHandler.Instance.currentSelection();
        parentHeightResetScript.OnHeightReset(index);
        selectionItemScript1.OnSelectedClick(index);
        //if (SNS_NFTHandler.Instance.ContentPanel.transform.childCount == 0)
        //{
    ...
    public void OnClickNFTTabButtonSub(int index)
    {
        ...
        selectionItemScript2.OnSelectedClick(index);
        SNS_NFTHandler.Instance.NftLoadingPenal.SetActive(true);
        SNS_NFTHandler.Instance.NoNftyet.SetActive(false);
        SNS_NFTHandler.Instance.NoNftyet.GetComponent<TMPro.TextMeshProUGUI>().text = string.Empty;
        SNS_NFTHandler.Instance.nftloading.SetActive(true);
        //   if (SNS_NFTHandler.Instance.ContentPanel.transform.childCount == 0)
        //  {
        //   OnScrollNFT();
        //   }
    ...
    {
        //GameObject[] objsNFTs = SNS_NFTHandler.Instance.ContentPanel.transform.childCount;
        print("come to async showing NFT");

        //if (UserRegisterationManager.instance.userRoleObj.NFTsURL.Count < SNS_NFTHandler.Instance.ContentPanel.transform.childCount)
        //{
        //  print("come to 2222");
        if (_OwnedNFTDataObj.NFTlistdata.list.Count > 0)
        {
            ...
            {
                GameObject L_ItemBtnObj = Instantiate(NFTImagePrefab, SNS_NFTHandler.Instance.ContentPanel.transform);
                Debug.Log("L_ItemBtnObj");
                if (_OwnedNFTDataObj.NFTstype.Count > i && _OwnedNFTDataObj.NFTstype[i] == 4)
                {
                    L_ItemBtnObj.gameObject.GetComponent<LoadAndShowNFTHandler>().VideoIcon.SetActive(true);
                }
                int locali = i;
                L_ItemBtnObj.gameObject.GetComponent<LoadAndShowNFTHandler>()._indexNumber = i;
                L_ItemBtnObj.gameObject.GetComponent<LoadAndShowNFTHandler>().isVisible = true;
        ...
        else
        {
            ...
            SNS_NFTHandler.Instance.NftLoadingPenal.SetActive(true);
            SNS_NFTHandler.Instance.NoNftyet.SetActive(true);
            SNS_NFTHandler.Instance.NoNftyet.GetComponent<TMPro.TextMeshProUGUI>().text = UITextLocalization.GetLocaliseTextByKey("NFT data not found");
            SNS_NFTHandler.Instance.nftloading.SetActive(false);
        }
    ...
    void SetupEditProfileScreen()
    {
        ...
        if (myProfileData.userProfile != null)
        {
            ...
            editProfileJobAdvanceInputfield.Text = SNS_APIResponseManager.DecodedString(myProfileData.userProfile.job);
            ...
            editProfileBioInputfield.Text = SNS_APIResponseManager.DecodedString(myProfileData.userProfile.bio);
            ...
            editProfileBioInputfield.transform.parent.GetComponent<InputFieldHightSetter>().OnValueChangeAfterResetHeight();
    ...
    public void OnClickEditProfileDoneButton()
    {
        ...
        if (myProfileData.userProfile != null)
        {
            ...
            job = SNS_APIResponseManager.DecodedString(myProfileData.userProfile.job);
            website = myProfileData.userProfile.website;
            bio = SNS_APIResponseManager.DecodedString(myProfileData.userProfile.bio);
    ...
    void EditProfileInfoCheckAndAPICalling()
    {
        ...
        if (checkEditNameUpdated == 1)
        {
            ...
            isEditProfileNameAlreadyExists = false;
            SNS_APIResponseManager.Instance.RequestSetName(username);
        }
        ...
        if (checkEditInfoUpdated == 1)
        {
            ...
            SNS_APIResponseManager.Instance.RequestUpdateUserProfile(uniqueUsername, gender, SNS_APIResponseManager.EncodedString(job), countryName, website, SNS_APIResponseManager.EncodedString(bio), tempTags);
        }
    ...
    IEnumerator WaitEditProfileGetUserDetails(bool isProfileUpdate)
    {
        ...
        if (!isProfileUpdate)
        {
            SNS_APIResponseManager.Instance.RequestGetUserDetails("EditProfileAvatar");//Get My Profile data 
        }
        else
        {
    ...
    public void OnEditBioButtonClick()
    {
        ...
        //bioEditInputField.transform.parent.GetComponent<InputFieldHightSetter>().OnValueChangeAndResetNormalInputField();
        //bioEditInputField.transform.parent.GetComponent<InputFieldHightSetter>().OnValueChangeAfterResetHeight();
    }
    ...
    public void ProfilePostPartShow(bool _resetProfileScreen = false)
    {
        ...
        if (_resetProfileScreen)
        {
            if (allPhotoContainer.childCount > 0)
            {
                ...
                //SNS_APIResponseManager.Instance.userId = myProfileData.id;
                //SNS_APIResponseManager.Instance.LoadMyPost();
            }
    ...
    public IEnumerator IERequestGetFeedsByUserId(int userId, int pageNum, int pageSize)
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Get((SNS_APIResponseManager.Instance.mainURL + SNS_APIResponseManager.Instance.url_GetFeedsByUserId + "/" + userId + "/" + pageNum + "/" + pageSize)))
        {
    ...
    {
        //FeedsManager.Instance.ProfileFollowerFollowingScreenSetup(0, topHaderUserNameText.text);
        FeedsManager.Instance.OnClickProfileFollowerButton();
        //if (SNS_APIResponseManager.Instance.profileAllFollowerRoot.data.rows.Count != myProfileData.followerCount)
        //{
        //    //FeedsManager.Instance.ProfileFollowerFollowingListClear();

        //    //FeedsManager.Instance.ShowLoader(true);
        //    FeedsManager.Instance.isProfileFollowerDataLoaded = false;
        //    SNS_APIResponseManager.Instance.RequestGetAllFollowersFromProfile(myProfileData.id.ToString(), 1, 50);
    ...
    {
        yield return new WaitForSeconds(1f);
        FeedsManager.Instance.isProfileFollowingDataLoaded = false;
        SNS_APIResponseManager.Instance.RequestGetAllFollowingFromProfile(myProfileData.id.ToString(), 1, 50);
    }

    //this method is used to profile Following button click.......
    public void OnClickFollowingButtton()
    {
        //FeedsManager.Instance.ProfileFollowerFollowingScreenSetup(1, topHaderUserNameText.text);
        FeedsManager.Instance.OnClickProfileFollowingButton();
        //if (SNS_APIResponseManager.Instance.profileAllFollowingRoot.data.rows.Count != myProfileData.followingCount)
        //{
        //    //FeedsManager.Instance.ProfileFollowerFollowingListClear();
