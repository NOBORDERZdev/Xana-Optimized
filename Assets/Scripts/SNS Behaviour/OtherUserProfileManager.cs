...
public class OtherUserProfileManager : MonoBehaviour
{
        ...
        if (backKeyManageList.Count > 0 && !isTempDirectMessageScreenOpen)
        {
            /*switch (backKeyManageList[backKeyManageList.Count - 1])
            {
                case "GroupDetailsScreen":
                   Debug.Log("OnDisable OtherUserProfileManager other user profile disable and clear data");
                    break;
                default:
                    break;
            }*/
            //Debug.Log("OnDisable OtherUserProfileManager feed screen enable");
    ...
    private void Update()
    {
        if (FeedsManager.Instance.otherPlayerProfileScreen.activeSelf)
        {
    ...
    public void LoadUserData(bool isFirstTime)
    {
        ...
        {
            ProfileScreenController.instance.followerBtn.interactable = false;
            ProfileScreenController.instance.followingBtn.interactable = false;
        ...
        if (isFirstTime)
        {
            ...
            if (singleUserProfileData.userProfile != null)
            {
                ...
                {
                    jobText.text = SNS_APIResponseManager.DecodedString(singleUserProfileData.userProfile.job);
                ...
                {
                    textUserBio.text = SNS_APIResponseManager.DecodedString(singleUserProfileData.userProfile.bio);
            ...
            {
                bool isAvatarUrlFromDropbox = SNS_APIResponseManager.Instance.CheckUrlDropboxOrNot(singleUserProfileData.avatar);
    ...
    public void UpdateUserTags()
    {
            ...
            if (ProfileScreenController.instance)
            {
                ...
                if (ProfileScreenController.instance.UserTagsParent.transform.childCount > singleUserProfileData.tags.Length)
                {
                    for (int i = 0; i < ProfileScreenController.instance.UserTagsParent.transform.childCount; i++)
                    {
                        ...
                        {
                            ProfileScreenController.instance.UserTagsParent.transform.GetChild(i).GetComponentInChildren<TextMeshProUGUI>().text = singleUserProfileData.tags[i];
                        }
                    }
                }
                else if (ProfileScreenController.instance.UserTagsParent.transform.childCount < singleUserProfileData.tags.Length)
                {
    ...
    public void ProfileAPiPagination()
    {
        ...
        if (profileMainScrollRectFasterEx.verticalNormalizedPosition < 0.01f && isFeedLoaded)
        {
            ...
            if (currentPageAllFeedWithUserIdRoot.Data.Rows.Count > 0)
            {
                ...
                isFeedLoaded = false;
                SNS_APIResponseManager.Instance.RequestGetFeedsByUserId(singleUserProfileData.id, (profileFeedAPiCurrentPageIndex + 1), 10, "OtherPlayerFeed");
            }
        }
    }
    ...
    public void AllFeedWithUserId(int pageNumb, bool _callFromFindFriendWithName = false)
    {
        ...
        FeedsManager.Instance.OnClickCheckOtherPlayerProfile(_callFromFindFriendWithName);
        ...
        for (int i = 0; i <= currentPageAllTextPostWithUserIdRoot.data.rows.Count; i++)
        {
            if (i < currentPageAllTextPostWithUserIdRoot.data.rows.Count)
            {
                if (!loadedMyPostAndVideoId.Contains(currentPageAllTextPostWithUserIdRoot.data.rows[i].id))
                {
                    ...
                    //GameObject userTagPostObject = Instantiate(userPostPrefab, userPostParent);
                    GameObject userTagPostObject = Instantiate(MyProfileManager.Instance.photoPrefab, parent);
            ...
            else//Case added to instantiate empty object at end of posts so last one wont get hidden behide bottom UI
        {
                ...
                //}
                //emptyFeedObjRef = Instantiate(MyProfileManager.Instance.EmptyFeedPrefab, userPostParent);
        }
    }
        ...
        IEnumerator WaitToFeedLoadedUpdate(int pageNum)
    {
        ...
        //userPostMainPart.GetComponent<ParentHeightHandler>().GetAndCheckMaxHeightInAllTab();

        SetupEmptyMsgForPhotoTab(false);//check for empty message.......
        userPostMainPart.GetComponent<ParentHeightHandler>().SetParentheight(userPostParent.GetComponent<RectTransform>().sizeDelta);
    ...
    public void OnClickPhotoButton()
    {
        ...
        ProfileScreenController.instance.otherUserButtonPanelScriptRef.OnSelectedClick(0);
    ...
    public void OnClickMovieButton()
    {
        ...
        ProfileScreenController.instance.otherUserButtonPanelScriptRef.OnSelectedClick(1);
    ...
    {
        SNS_APIController.Instance.RemoveFollowedUserFromHot(singleUserProfileData.id);
            ...
            switch (backKeyManageList[backKeyManageList.Count - 1])
            {
                case "FollowerFollowingListScreen":
                   Debug.Log("Last Comes from Follower Following list screen my profile");
                    ...
                    FeedsManager.Instance.profileFollowerFollowingListScreen.SetActive(true);
                    FeedsManager.Instance.footerCan.GetComponent<HomeFooterTabCanvas>().SetDefaultButtonSelection(4);
                    RefreshDataFollowerAndFollowingScreen();
                    break;
                case "HotTabScreen":
                   Debug.Log("Last Comes from Hot or discover tab Screen");
                    FeedsManager.Instance.RemoveUnFollowedUserFromFollowingTab();
                    FeedsManager.Instance.feedUiScreen.SetActive(true);
                    break;
                case "FollowingTabScreen":
                   Debug.Log("Last Comes from Following tab screen");
                    FeedsManager.Instance.RemoveUnFollowedUserFromFollowingTab("FollowingTabScreen");
                    FeedsManager.Instance.feedUiScreen.SetActive(true);
                    break;
                case "GroupDetailsScreen":
                   Debug.Log("Last Comes from message module group details screen");
                    ...
                    RemoveAndCheckBackKey();
                    FeedsManager.Instance.footerCan.GetComponent<HomeFooterTabCanvas>().OnClickWorldButton();
                    break;
                default:
                    FeedsManager.Instance.feedUiScreen.SetActive(true);
                    break;
    ...
    void RefreshDataFollowerAndFollowingScreen()
    {
        if (currentFollowerItemScript != null)
        {
            if (currentFollowerItemScript.followerRawData.isFollowing != isFollowFollowing)
            {
                ...
                FollowingItemController followingItemController = FeedsManager.Instance.profileFollowingItemControllersList.Find((x) => x.followingRawData.following.id == userId);
        ...
        else if (currentFollowingItemScript != null)
        {
            if (currentFollowingItemScript.followingRawData.isFollowing != isFollowFollowing)
            {
                ...
                FollowerItemController followerItemController = FeedsManager.Instance.profileFollowerItemControllersList.Find((x) => x.followerRawData.follower.id == userId);
    ...
    public void OnClickFollowUserButton()
    {
       Debug.Log("OnClickFollowUser id" + singleUserProfileData.id.ToString() + " :FollowText:" + followText.text);
        ...
        {
            //unfollow.......
            SNS_APIResponseManager.Instance.RequestUnFollowAUser(singleUserProfileData.id.ToString(), "OtherUserProfile");
            //ProfileScreenController.instance.followProfileBtn.GetComponentInChildren<TextMeshProUGUI>().text = "Follow";
            ProfileScreenController.instance.followProfileBtn.GetComponentInChildren<UITextLocalization>().LocalizeTextText("Follow");
        }
        else
        {
            //follow.......
            SNS_APIResponseManager.Instance.RequestFollowAUser(singleUserProfileData.id.ToString(), "OtherUserProfile");
            //ProfileScreenController.instance.followProfileBtn.GetComponentInChildren<TextMeshProUGUI>().text = "Unfollow";
            ProfileScreenController.instance.followProfileBtn.GetComponentInChildren<UITextLocalization>().LocalizeTextText("Unfollow");
        }
    }
    ...
    public void RequestGetUserDetails(SingleUserProfileData singleUserProfileData1, bool _callFromFindFriendWithName = false)
    {
        ...
        SNS_APIResponseManager.Instance.RequestGetFeedsByUserId(singleUserProfileData1.id, 1, 40, "OtherPlayerFeed", _callFromFindFriendWithName);
    ...
    public void RequestGetUserDetails(int id)
    {
        ...
        SNS_APIResponseManager.Instance.RequestGetFeedsByUserId(id, 1, 40, "OtherPlayerFeed");
