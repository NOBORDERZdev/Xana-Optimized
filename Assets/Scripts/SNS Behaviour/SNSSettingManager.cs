...
public class SNSSettingManager : MonoBehaviour
{
    public static SNSSettingManager Instance;
    ...
    {
        MyProfileManager.Instance.CreateFirstFeedPlusAnimStop(true);
    ...
    {
        OnClickSettingOpen();
        //MyProfileManager.Instance.CreateFirstFeedPlusAnimStop(false);//check profile post empty or not and start bottom create plus icon anim
    }
    ...
    public void OnClickPersonalInformationButton()
    {
        ...
        if (MyProfileManager.Instance.myProfileData.id == 0)
        {
            FeedsManager.Instance.ShowLoader(true);
            SNS_APIResponseManager.Instance.RequestGetUserDetails("MyAccount");//Get My Profile data    
        }
        else
        {
            SetUpDataOfPersonalInfo(MyProfileManager.Instance.myProfileData.email, MyProfileManager.Instance.myProfileData.phoneNumber);
    ...
    public void LogoutSuccess()
    {
        ...
        {
            MyProfileManager.Instance.ClearAndResetAfterLogout();
            ...
            FeedsManager.Instance.feedController.ResetFeedController();
            FeedsManager.Instance.ClearAllFeedDataAfterLogOut();
            FeedsManager.Instance.footerCan.GetComponent<HomeFooterTabCanvas>().OnClickHomeButton();
            FeedsManager.Instance.footerCan.GetComponent<HomeFooterTabCanvas>().CheckLoginOrNotForFooterButton();
