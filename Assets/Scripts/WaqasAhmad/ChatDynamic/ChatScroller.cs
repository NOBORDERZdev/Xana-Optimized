...
public class ChatScroller : MonoBehaviour, IEnhancedScrollerDelegate, IBeginDragHandler, IEndDragHandler
{
    /// <summary>
    ...
    public EnhancedScrollerCellView GetCellView(EnhancedScroller scroller, int dataIndex, int cellIndex)
    {
        ...
        Debug.Log("Waqas: GetCellView Called");
        CellView cellView = scroller.GetCellView(cellViewPrefab) as CellView;

        // set the name of the game object to the cell's data index.
        // this is optional, but it helps up debug the objects in 
        // the scene hierarchy.
        cellView.name = "Msg_ " + dataIndex.ToString();

        // in this example, we just pass the data to our cell's view which will update its UI
        //cellView.SetData(_data[dataIndex]);
        //cellView.GetComponent<FeedData>().SetFeedUiController(this.GetComponent<FeedScroller>());
        ChatMsgDataHolder _newMsg = cellView.GetComponent<ChatMsgDataHolder>();
        _newMsg.SetRequireData(_data[dataIndex].message, _data[dataIndex].message_id.ToString(), _data[dataIndex].userId, _data[dataIndex].isMessageBlocked);

        string userNameModification = "";
        if (_data[dataIndex].isGuest)
            userNameModification = _data[dataIndex].guestusername;
        else
        {
            userNameModification = _data[dataIndex].name;
            if (string.IsNullOrEmpty(userNameModification))
                userNameModification = _data[dataIndex].username;
        }

        if(!string.IsNullOrEmpty(userNameModification))
            _newMsg.DisplayMsg(userNameModification, _data[dataIndex].message);

        //cellView.GetComponent<ChatMsgDataHolder>().SetRequireData(_data[dataIndex].message, "", _data[dataIndex].userId, _data[dataIndex].isMessageBlocked);
