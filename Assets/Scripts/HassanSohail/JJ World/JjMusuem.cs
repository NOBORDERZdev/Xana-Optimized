...
public class JjMusuem : MonoBehaviour
{
    ...
    public void SetPlayerPos(JJMussuemEntry mussuemEntry) {
        player = ReferrencesForGameplay.instance.MainPlayerParent.GetComponent<PlayerController>();
        if (ReferrencesForGameplay.instance.m_34player.gameObject.GetComponentInChildren<PhotonView>().IsMine)
        {
            switch (mussuemEntry)
            {
    ...
    CheckAgain:
        ...
        ReferrencesForGameplay.instance.MainPlayerParent.transform.eulerAngles = destinationPoint.eulerAngles;
