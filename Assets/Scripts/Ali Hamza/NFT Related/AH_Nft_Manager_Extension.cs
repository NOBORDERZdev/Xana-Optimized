...
namespace Toyota
{
    public class AH_Nft_Manager_Extension : MonoBehaviour
    {
        ...
        public static AH_Nft_Manager_Extension Instance { get; private set; }
                ...
                for (int j = 0; j < worldData.Count; j++)
                {
                    if (i == worldData[j].index - 1)
                    {
                        ...
                        if (worldData[j].media_type == "IMAGE")
                        {
                            ...
                            NftPlaceholderList[i].GetComponent<AH_VideoAndImage_Extension>().InitData(worldData[j].asset_link + compersionPrfex, null, worldInfos[i].pmyRatio, PMY_DataType.Image, PMY_VideoTypeRes.none);
                        ...
                        else if (worldData[j].media_type == "VIDEO" || worldData[j].media_type == "LIVE")
                        {
                            ...
                            if (worldPlayingVideos) // to play video's in world
                            {
                                if (worldData[j].youtubeUrlCheck && !string.IsNullOrEmpty(worldData[j].youtubeUrl))  //for Live Video 
                                {
                                    ...
                                    NftPlaceholderList[i].GetComponent<AH_VideoAndImage_Extension>().InitData(null, worldData[j].youtubeUrl, worldInfos[i].pmyRatio, PMY_DataType.Video, PMY_VideoTypeRes.islive);
                                }
                                else if (!worldData[j].youtubeUrlCheck && !string.IsNullOrEmpty(worldData[j].youtubeUrl))  // for Prerecorded video
                                {
                                    ...
                                    NftPlaceholderList[i].GetComponent<AH_VideoAndImage_Extension>().InitData(null, worldData[j].youtubeUrl, worldInfos[i].pmyRatio, PMY_DataType.Video, PMY_VideoTypeRes.prerecorded);
                                }
                                else if (!string.IsNullOrEmpty(worldData[j].asset_link))
                                {
                                    ...
                                    NftPlaceholderList[i].GetComponent<AH_VideoAndImage_Extension>().InitData(null, worldData[j].asset_link + compersionPrfex, worldInfos[i].pmyRatio, PMY_DataType.Video, PMY_VideoTypeRes.aws);
                                }
                                isWithDes = true;
                                worldInfos[i].Title = worldData[j].title;
                                worldInfos[i].Aurthor = worldData[j].authorName;
                        ...
                        else if(worldData[j].media_type == "PDF")
                        {
                            ...
                            NftPlaceholderList[i].GetComponent<AH_VideoAndImage_Extension>().InitData(worldData[j].thumbnail, null, worldInfos[i].pmyRatio, PMY_DataType.PDF, PMY_VideoTypeRes.none);
                        }
                    ...
                    else
                    {
                        if (j == worldData.Count - 1)
                        {
                            ...
                            NftPlaceholderList[i].GetComponent<AH_VideoAndImage_Extension>().TurnOffAllImageAndVideo();
        ...
        {
            worldInfos[obj.GetComponent<AH_VideoAndImage_Extension>().id].VideoLink = precorderUrl;
            worldInfos[obj.GetComponent<AH_VideoAndImage_Extension>().id].videoType = PMY_VideoTypeRes.prerecorded;
            obj.GetComponent<AH_VideoAndImage_Extension>().InitData(null, precorderUrl, worldInfos[obj.GetComponent<AH_VideoAndImage_Extension>().id].pmyRatio, PMY_DataType.Video, PMY_VideoTypeRes.prerecorded);
        }

        public void LoadLiveIfFirstTimeNotLoaded(GameObject obj, string url)
        {
            if (obj.GetComponent<AH_VideoAndImage_Extension>() != null)
            {
                worldInfos[obj.GetComponent<AH_VideoAndImage_Extension>().id].VideoLink = url;
                worldInfos[obj.GetComponent<AH_VideoAndImage_Extension>().id].videoType = PMY_VideoTypeRes.islive;
                obj.GetComponent<AH_VideoAndImage_Extension>().InitData(null, url, worldInfos[obj.GetComponent<AH_VideoAndImage_Extension>().id].pmyRatio, PMY_DataType.Video, PMY_VideoTypeRes.islive);
            }
        }
        ...
        public void SetInfo(PMY_Ratio ratio, string title, string aurthur, string des, string url, Texture2D image, PMY_DataType type, string videoLink, PMY_VideoTypeRes videoType, string pdfURL, QuizData quizData ,int nftId = 0, AH_VideoAndImage_Extension.RoomType roomType = AH_VideoAndImage_Extension.RoomType.Gallery)
        {
            nftTitle = title;
            _Ratio = ratio;
            _Title = title;
        ...
        public void SendCallAnalytics(int id = -1, AH_VideoAndImage_Extension.RoomType roomType = AH_VideoAndImage_Extension.RoomType.Gallery)
        {
            // For firebase event
            string eventName = "";
            switch (roomType)
