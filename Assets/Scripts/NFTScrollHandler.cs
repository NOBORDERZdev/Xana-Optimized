...
public class NFTScrollHandler : MonoBehaviour
{
    ...
    public async void EquipBtnClicked()
    {
        ...
        if (subButtonTextToCheck == "Equip")
        {
            ...
            if (!_IsInOwnerShip)
            {
                SNS_NFTHandler.Instance.NftTransferedPanel.SetActive(true);
                return;
            }
            else
            {
    ...
    public void WaitForOpenWorldPage()
    {
        ...
        ScrollController.transform.parent.GetComponent<NFTScrollHandler>().enabled = true;
    }
