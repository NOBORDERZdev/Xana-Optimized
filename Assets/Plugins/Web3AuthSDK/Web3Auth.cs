...
public class Web3Auth : MonoBehaviour
{
    ...
    private string StartLocalWebserver()
    {
        ...
        var redirectUrl = $"http://localhost:{Utilss.GetRandomUnusedPort()}";
    ...
    private async void request(string path, LoginParams loginParams = null, Dictionary<string, object> extraParams = null)
    {
...
#elif UNITY_WEBGL
        this.initParams["redirectUrl"] = Utilss.GetCurrentURL();
#endif
        ...
        if (!string.IsNullOrEmpty(loginId))
        {
            ...
            Utilss.LaunchUrl(uriBuilder.ToString(), this.initParams["redirectUrl"].ToString(), gameObject.name);
        }
        else
        {
    ...
    public void setResultUrl(Uri uri)
    {
        ...
        Dictionary<string, string> queryParameters = Utilss.ParseQuery(uri.Query);
    ...
    public Web3UserInfo getUserInfo()
    {
        if (web3AuthResponse == null)
