...
#pragma warning restore 0162
    ...
    public static string VideoPropertiesStr;
    public static double VideoLength(string path)
    {
        int length = 0;
        GetVideoProperties(path);
        if (!string.IsNullOrEmpty(VideoPropertiesStr))
        {
            string[] properties = VideoPropertiesStr.Split('>');
            if (!string.IsNullOrEmpty(properties[2].Trim()))
            {
                length = int.Parse(properties[2].Trim());
                Debug.Log("Vkideo Length" + length);
            }
        }
        return length;
    }
    public static VideoProperties GetVideoProperties(string videoPath)
    {
		...
		VideoPropertiesStr=value;
		Debug.Log("Video Properties : " + value);
#elif !UNITY_EDITOR && UNITY_IOS
