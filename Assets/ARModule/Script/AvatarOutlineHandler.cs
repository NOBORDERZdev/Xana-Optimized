...
public class AvatarOutlineHandler : MonoBehaviour
{
    public static AvatarOutlineHandler Instance;
    ...
    public void AvatarSelectionBorder(bool isActive)
    {
        ...
        if (!isMainAvatarOnActionScreen)
        {
            if (!isActive)
            {
                ARFaceModuleHandler.Instance.OnDeleteItemObject();
            }
            else
            {
                ARFaceModuleHandler.Instance.currentSelectedItemObj = this.gameObject;
            }
            ARFaceModuleHandler.Instance.OnDeleteScreenActive(isActive);
        }
    }
}